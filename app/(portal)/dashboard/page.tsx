    ...
    <div className="p-4 md:p-8 max-w-5xl mx-auto">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mb-6 md:mb-8">
        <div>
          ...
          <p className="text-slate-400 text-sm mt-0.5">Track your content generation activity</p>
        </div>
      ...
      {subscription?.status !== 'active' && plan === 'free' && used >= limit - 1 && (
        <div className="bg-gradient-to-r from-sky-500/20 to-cyan-500/20 border border-sky-500/30 rounded-2xl p-4 mb-6 md:mb-8 flex items-center justify-between gap-3">
          <div className="flex items-center gap-3">
            ...
            <div className="md:hidden divide-y divide-white/5">
              {history.map((h: { id: string; topic: string; formats_count?: number; total_words?: number; created_at: string }) => (
                <div key={h.id} className="px-4 py-3">
                  <p className="text-white text-sm font-medium truncate mb-1">{h.topic}</p>
                  <div className="flex items-center gap-3 text-xs text-slate-400">
                    <span>{h.formats_count ?? 16} formats</span>
                    <span>·</span>
                    <span>{(h.total_words ?? 0).toLocaleString()} words</span>
                    <span>·</span>
